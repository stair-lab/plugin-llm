...
class ProcessedDataset:
    def __init__(self, name, base_model_name):
        ...
        if name == "adidas":
            ...
            self.data = load_dataset(
                "web_nlg", "webnlg_challenge_2017", trust_remote_code=True
            )
        else:
    ...
    def preprocess_sep_input_target(
        self, examples, tokenizer, input_size, target_size, add_extra_tokens=False
    ):
        ...
        model_inputs = tokenizer(
            inputs, max_length=input_size, truncation=True, padding="max_length"
        )
        labels = tokenizer(
            targets, max_length=target_size, truncation=True, padding="max_length"
        )
    ...
    def preprocess_concat_input_target(
        self, examples, tokenizer, input_size, target_size
    ):
        ...
        concatenated_input_ids = [
            list(input_seq) + list(target_seq)
        ...
        concatenated_attention_mask = [
            list(input_mask) + list(target_mask)
            for input_mask, target_mask in zip(
                tokenized_inputs["attention_mask"], tokenized_targets["attention_mask"]
            )
        ]
        ...
        labels = []
        for input_len, target_seq in zip(
            [input_size] * len(inputs), tokenized_targets["input_ids"]
        ):
    ...
    def preprocess_preconcat_input_target(
        self, examples, tokenizer, input_size, target_size
    ):
        # Extract the meaning representations (MR) and human references (target text) from the examples
        inputs = examples["meaning_representation"]
        ...
        labels = []
        for comb_seq, target_seq in zip(
            tokenized_sentences["input_ids"], tokenized_targets["input_ids"]
        ):
    ...
    def direct_tokenize(self, examples, tokenizer, text_size):
        return tokenizer(
            ...
            text_target=examples["human_reference"],
            padding="max_length",
            truncation=True,
    ...
    def mapped_tokenize(self, tokenizer, input_size, target_size):
        ...
        self.data = self.data.map(
            self.preprocess_preconcat_input_target,
            batched=True,
            fn_kwargs=fn_kwargs_dict,
            # remove_columns=["meaning_representation", "human_reference"]
        )
    ...
    def mapped_tokenize_for_evaluate(self, tokenizer, input_size):
        ...
        print("there")
        self.data = self.data.map(
            self.preprocess_input_for_evaluate,
            batched=True,
            fn_kwargs=fn_kwargs_dict,
            # remove_columns=["meaning_representation", "human_reference"]
        )
    ...
    def filter_rows_in_list(self, col_name, valid_list):
        ...
        self.data = DatasetDict(
            {
                split: dataset.filter(lambda example: example[col_name] in valid_list)
    ...
    def split_data(self, split_key_name, test_size=0.2, random_state=42):
        ...
        train_mrs, hypervalidation_mrs = train_test_split(
            unique_mrs, test_size=test_size, random_state=random_state
        )
        ...
        hypervalidation_dataset = DatasetDict(
            {f"hyper{split_key_name}": Dataset.from_pandas(hypervalidation_df)}
        )
        hypervalidation_dataset = self.remove_cols_in_df(
            hypervalidation_dataset, "__index_level_0__"
        )

        self.data[split_key_name] = train_dataset[split_key_name]
        self.data[f"hyper{split_key_name}"] = hypervalidation_dataset[
            f"hyper{split_key_name}"
        ]
