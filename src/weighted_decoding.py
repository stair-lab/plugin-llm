...
def main():
    ...
    if(args.model_type == 'gpt2'):
        if(config['plugin_model']['gpt2']['name']):
            ...
            model = CustomGPT2CombinedModelBatch.from_pretrained(config['plugin_model']['gpt2']['name'], base_model, args.new_model_weight)
        else:
            logger.info('Loading provided config based model')
            gpt2_tmp_config = GPT2Config(**config['plugin_model']['gpt2'])
            model = CustomGPT2CombinedModelBatch(gpt2_tmp_config, base_model, args.new_model_weight)
    elif(args.model_type == 'llama'):
        if(config['plugin_model']['llama']['name']):
            logger.info('Loading pretrained model')
            model = CustomLlamaCombinedModelBatchSeparate.from_pretrained(config['plugin_model']['llama']['name'], tokenizer, args.new_model_weight)
        else:
            logger.info('Loading provided config based model')
            llama_tmp_config = LlamaConfig(**config['plugin_model']['llama'])
            model = CustomLlamaCombinedModelBatchSeparate(llama_tmp_config, tokenizer, args.new_model_weight)
