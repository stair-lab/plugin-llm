...
def plot_style_word_probabilities(
    plugin_probs,
    base_probs,
    style_token_ids,
    style_words,
        ...
        word_probs.append(
            {
                ...
                "base_prob": base_probs[
                    0, tid
                ].item(),  # Already normalized over full vocab
                "combined_prob": plugin_probs[
                    0, tid
                ].item(),  # Already normalized over full vocab
    ...
    plt.bar(
        x - width / 2,
        [wp["base_prob"] for wp in word_probs],
        width,
        label="Base Model",
        color="lightblue",
        alpha=0.6,
    ...
    plt.bar(
        x + width / 2,
        [wp["combined_prob"] for wp in word_probs],
        width,
        label="Combined",
        color="lightcoral",
        alpha=0.6,
    ...
    plt.title(
        f'Sample {selected_id} - Token Probabilities at Generation Step {step}\nGenerated Token: "{next_token}"'
    )
...
class DictDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "meaning_representation": example[
                "meaning_representation"
            ],  # Remove batch dimension
        }


def custom_generate_original(
    model,
    input_ids,
    attention_mask,
    max_length,
    ...
    top_p=1,
    bb_model=None,
    new_model_weight=None,
    style_token_ids=None,
    style_words=None,
    selected_id=None,
):
    ...
    finished_sequences = torch.zeros(input_ids.size(0), dtype=torch.bool).to(
        input_ids.device
    )
    k = 0

    position_ids = get_position_ids_left_padded(
    ...
    for step in range(max_length):
        ...
        if bb_model:
            ...
            if style_token_ids and style_words:
                ...
                word_probs = []
                for word, _ in style_words:
                    space_token = tokenizer(" " + word, add_special_tokens=False)[
                        "input_ids"
                    ]
                    if len(space_token) == 1:
                        tid = space_token[0]
        ...
        next_token = torch.where(
            finished_sequences.unsqueeze(-1), tokenizer.pad_token_id, next_token
        )
        ...
        new_attention_mask = torch.ones((attention_mask.shape[0], 1)).to(
            input_ids.device
        )
        ...
        last_values = position_ids[
            :, -1
        ]  # This gets the last value of each row (shape: m)
        new_values = last_values + 1  # Increment each last value by 1
        new_values = new_values.unsqueeze(
            1
        )  # Reshape to (m, 1) to concatenate with the tensor
        position_ids = torch.cat([position_ids, new_values], dim=1)

        if finished_sequences.all():
            eos_tensor = torch.full(
                (input_ids.size()[0], max_length - step - 1), tokenizer.eos_token_id
            ).to(input_ids.device)
    ...
    if all_steps_data:
        ...
        df.to_csv(
            f"./probability_plots/sample_{selected_id}_all_steps_probs.csv", index=False
        )

    return generated_ids[:, input_size:]


def get_eval_dat_per_model(
    tokenizer,
    eval_model,
    dat_loader,
    device,
    input_size,
    meaning_to_references,
    max_length=128,
    ...
    for b, batch in tqdm(enumerate(dat_loader)):
        ...
        if bb_model:
            generated_ids = custom_generate_original(
                eval_model.module,
                input_ids=input_ids,
                attention_mask=attention_mask,
    ...
    predicted_text_list = tokenizer.batch_decode(
        generated_ids_tensor, skip_special_tokens=True
    )
    ...
    return {
        "predictions": predicted_text_list,
        "meaning_representations": mrs,
            ...
            if base_model_name == "gpt2-medium":
                ...
                for j, i in enumerate(ic_ids):
                    static_str_from_prompt = (
                        "Convert the following facts into a coherent sentence:\n\n"
                    )
...
def main():
    ...
    parser.add_argument(
        "--evaluate_model_name", type=str, required=True, help="Moddel to evaluate"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="GPU id to use for evaluation (defaults to 0)",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Weight for the new model in linear combination",
    )
    parser.add_argument(
        "--len_context", type=int, default=0, help="Number of in-context examples"
    )
    ...
    file_handler = logging.FileHandler(
        os.path.join(config["logs_dir"], model_name + "_evaluate.log")
    )
    ...
    if args.base_model_name:
        try:
            tokenizer = AutoTokenizer.from_pretrained(
                os.path.join(config["models_dir"], args.base_model_name)
            )
            logger.info("Loaded tokenizer from local base model")
        except OSError:
            tokenizer = AutoTokenizer.from_pretrained(
                os.path.join(args.base_model_name)
            )
            logger.info("Loaded tokenizer from Huggingface base model")
    else:
        try:
            tokenizer = AutoTokenizer.from_pretrained(
                os.path.join(config["models_dir"], args.evaluate_model_name)
            )
            logger.info("Loaded tokenizer from local evaluate model")
        except OSError:
            tokenizer = AutoTokenizer.from_pretrained(
                os.path.join(args.evaluate_model_name)
            )
    ...
    if config["data"]["dataset_name"] == "adidas":
        ...
        logger.info(
            f"processed web_nlg data with base model prompt {config['model']['base_model_for_prompt']}"
        )
        ...
        dataset = process_e2e_nlg_cleaned(
            dataset, config["model"]["base_model_for_prompt"]
        )
        logger.info(
            f"processed e2e data with base model prompt {config['model']['base_model_for_prompt']}"
        )
        ...
        logger.info(
            f"processed common_gen data with base model prompt {config['model']['base_model_for_prompt']}"
        )

    # getting prompt for in-context generation
    context = get_ic_prompt(
        args.len_context,
        dataset["validation"],
        config["model"]["base_model_for_prompt"],
        config["data"]["dataset_name"],
        logger,
    )
    ...
    for entry in dataset:
        meaning_to_references[context + entry["meaning_representation"]].append(
            entry["human_reference"]
        )
    ...
    unique_dataset = DictDataset(
        [{"meaning_representation": last_mr}], tokenizer, input_size
    )
    ...
    eval_dataloader = DataLoader(
        unique_dataset, batch_size=args.batch_size, shuffle=False
    )
    ...
    style_token_ids = get_token_ids_for_words(
        [word for word, _ in style_words], tokenizer, logger
    )

    output = get_eval_dat_per_model(
        tokenizer,
        model,
        eval_dataloader,
        device,
    ...
    bleu_score = bleu_metric.compute(
        predictions=output["predictions"], references=output["references"]
    )
    ...
    cider_score, _ = cider_metric.compute_score(
        res={
            k: [v]
            for k, v in dict(
                zip(list(range(len(output["predictions"]))), output["predictions"])
            ).items()
        },
    ...
    for i in range(len(output["predictions"])):
        ...
        if (
            len(pred.split(" ")) >= 6
        ):  # nist by default assumes 5 n-grams. Taking 6, just to be safe.s
            filtered_preds.append(pred)
            filtered_refs.append(output["references"][i])

    nist_score = nist_metric.evaluate(
