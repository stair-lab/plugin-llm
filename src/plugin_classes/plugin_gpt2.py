...
from utils.commons import (
    get_position_ids_text_completion_left_padded,
    get_position_ids_left_padded,
)
...
class CustomGPT2ModelBatch(GPT2LMHeadModel):
    ...
    def forward(self, input_ids, attention_mask=None, labels=None):
        ...
        with torch.no_grad():
            outputs_base = self.ft_model.forward(
                input_ids, attention_mask=attention_mask, position_ids=position_ids
            )
        ...
        normalized_probabilities = weighted_probabilities / weighted_probabilities.sum(
            dim=-1, keepdim=True
        )

        modified_logits = torch.log(
            normalized_probabilities + 1e-8
...
class CustomGPT2Model(GPT2LMHeadModel):
    ...
    def forward(self, input_ids, attention_mask=None, labels=None):
        ...
        modified_logits = torch.empty(
            input_ids.size(0), self.target_size, self.ft_model.config.vocab_size
        ).to(
            input_ids.device
        )  # Empty tensor of the desired final size
