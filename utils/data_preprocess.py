...
def parse_mr_to_string(mr):
    ...
    formatted_string = ",\n".join(
        [f"{key} -- {value}" for key, value in key_value_pairs]
    )
...
def process_e2e_nlg_cleaned(dataset, base_model_name):
    ...
    dataset["validation"] = dataset["validation"].map(
        add_input_prompt, fn_kwargs=fn_kwargs_dict
    )
...
def process_web_nlg(
    ...
    train_dataset = dataset["train"].filter(
        lambda x: filter_by_category(x, train_categories)
    )

    # Filter 'dev' and 'test' sets to only include the "Food" category
    dev_dataset = dataset["dev"].filter(
        lambda x: filter_by_category(x, test_categories)
    )
    ...
    def select_good_comment(example):
        for i, comment in enumerate(example["lex"]["comment"]):
            if comment == "good":
                return {
                    "human_reference": example["lex"]["text"][i]
                }  # Pick the sentence marked 'good'
        return {
            "human_reference": ""
        }  # Default to the first sentence if none are marked 'good'
    ...
    combined_dev_test = DatasetDict(
        {"validation": group1_dataset, "test": group2_dataset}
    )
    ...
    train_dataset = train_dataset.remove_columns(
        [
            ...
            if col not in ["meaning_representation", "human_reference"]
        ]
    )
    combined_dev_test["validation"] = combined_dev_test["validation"].remove_columns(
        [
            col
            for col in combined_dev_test["validation"].column_names
            if col not in ["meaning_representation", "human_reference"]
        ]
    )
    combined_dev_test["test"] = combined_dev_test["test"].remove_columns(
        [
            col
            for col in combined_dev_test["test"].column_names
            if col not in ["meaning_representation", "human_reference"]
        ]
    )
    ...
    def filter_empty_human_reference(example):
        return (
            example["human_reference"] is not None
            and example["human_reference"].strip() != ""
        )

    def filter_empty_meaning_representation(example):
        return (
            example["meaning_representation"] is not None
            and example["meaning_representation"].strip() != ""
        )

    train_dataset = train_dataset.filter(filter_empty_human_reference)
    combined_dev_test["validation"] = combined_dev_test["validation"].filter(
        filter_empty_human_reference
    )
    combined_dev_test["test"] = combined_dev_test["test"].filter(
        filter_empty_human_reference
    )

    train_dataset = train_dataset.filter(filter_empty_meaning_representation)
    combined_dev_test["validation"] = combined_dev_test["validation"].filter(
        filter_empty_meaning_representation
    )
    combined_dev_test["test"] = combined_dev_test["test"].filter(
        filter_empty_meaning_representation
    )
...
def process_common_gen(dataset, base_model_name):
    ...
    def filter_target_and_concepts(example):
        ...
        concepts_does_not_contain_man_or_woman = all(
            c not in ["man", "woman"] for c in example["concepts"]
        )
...
def process_adidas(dataset, base_model_name):
    ...
    dataset = DatasetDict({"validation": val_dataset, "test": test_dataset})

    def add_input_prompt(examples, base_model_name):
        inp = examples["meaning_representation"]
        if "gpt2-medium" in base_model_name:
            prefix_str = (
                "Given the following attributes of a product, write a description.\n\n"
            )
            suffix_str = "\n\nDescription:"
            new_input = prefix_str + inp + suffix_str
        elif "gpt2-xl" in base_model_name:
