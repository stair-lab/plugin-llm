...
def get_position_ids_left_padded(input_ids, attention_mask):
    ...
    position_ids = torch.where(
        attention_mask == 1, position_ids, torch.zeros_like(position_ids)
    )
    return position_ids.long()


def get_position_ids_text_completion_left_padded(
    input_ids, attention_mask, split_point=64
):
    """
    ...
    mr_position_ids = (
        torch.arange(split_point, dtype=torch.long)
        .unsqueeze(0)
        .repeat(input_ids.size(0), 1)
        .to(input_ids.device)
    )
    ...
    mr_position_ids = torch.where(
        mr_attention_mask == 1, mr_position_ids, torch.zeros_like(mr_position_ids)
    )
    ...
    target_position_ids = (
        torch.arange(target_seq_len, dtype=torch.long)
        .unsqueeze(0)
        .repeat(input_ids.size(0), 1)
        .to(input_ids.device)
    )
    ...
    target_position_ids += mr_seq_lengths.unsqueeze(1).repeat(
        1, target_position_ids.size()[1]
    )

    # Set position IDs for padding tokens in human reference to 0
    target_position_ids = torch.where(
        target_attention_mask == 1,
        target_position_ids,
        torch.zeros_like(target_position_ids),
    )
